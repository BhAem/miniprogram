...
Page({
  ...
  onShareAppMessage: function () {
    var that = this
    let url = encodeURIComponent('../../pages/demand/demand?obj=' + this.data.options_obj + '&user=' + this.data.options_user);
    return {
      title: "乡亲们，俺想要-"+that.data.demand.title,
    ...
    if (app.globalData.myopenid == null) {
      ...
      if (!this.data.collect_flag) {
        ...
        db.collection('collection').where({
          ...
          res => {
            if (res.data[0].c_id.indexOf(this.data._id) == -1) {
              db.collection('collection').where({
                _openid: app.globalData.myopenid
              }).update({
                data: {
