...
Page({
  ...
  async getMyDemand() {
    await wx.cloud.callFunction({
      ...
      this.setData({
        myDemand: Array.from(new Set(this.data.myDemand.concat(t).map(item => JSON.stringify(item)))).map(item => JSON.parse(item)),
        triggered: false
      })
    }).catch(err => {})
},
  ...
  showDetail: function(e) {
    var idx = e.currentTarget.dataset.index;
      var good = this.data.myDemand[idx]
  ...
  edit: function(e) {
    ...
    if (app.globalData.myopenid == null) {
      ...
      var good = this.data.myDemand[idx]
        good = JSON.stringify(good)
        wx.navigateTo({
          url: '../../pages/check2/check2?obj=' + good,
        })
    }
  },
