...
Page({
  ...
  // 选择图片 &&&
  addPic1: function (e) {
    // console.log("205: ", this.data.imgbox.length)
    ...
    wx.chooseImage({
      ...
      success: function (res) {
        ...
        // console.log("214: ", res.tempFilePaths.length)
  ...
  imgbox: function (e) {
   ...
   btnSub(res1) {
      ...
      db.collection('commodity').doc(this.data._id).get().then(
        res2 => {
          wx.showLoading({
            title: '修改中',
          })
        ...
        Promise.all(promiseArr).then(res4 => {
          ...
          var price = res1.detail.value.price;
        ...
        err => {
          ...
          for (let i = 0; i < this.data.imgbox.length; i++) {
            ...
            if (str != 'c') {
              //在每次上传的时候，就往promiseArr里存一个promise，只有当所有的都返回结果时，才可以继续往下执行
              promiseArr.push(new Promise((resolve, reject) => {
                let suffix = /\.[^\.]+$/.exec(this.data.imgbox[i])[0]; // 正则表达式，获取文件扩展名
                wx.cloud.uploadFile({
                  cloudPath: new Date().getTime()+ i.toString() + suffix,
                  filePath: filePath, // 文件路径
                }).then(res3 => {
